...
function ajoutVente(element)
{
    ...
    let vente = input.value;            
            ...
            let link =`${superLink}/addVenteVignette/${idb}/${vente}`;
...
function getAllCommande()
{
    ...
    xhr.open("GET",superLink+"/Json/vignette/billet",true);
    xhr.send();
    ...
    for(let i=0 ;i<inputs.length ;i++)
    {
        ...
        if(element.value != "" && !element.disabled) 
        {
            if (vente<=stock && vente>=1) 
            {
                ...
                let link =`${superLink}/addVenteVignette/${idc}/${vente}`;
                xhttp.open("GET",link,true);
                xhttp.send();
            }
            else
    ...
    for(let i=0 ;i<inputs.length ;i++)
    {
        ...
        if(element.value != "" && !element.disabled)
        {
            if (vente<=stock && vente>=1)
            {
                ...
                let link =`${superLink}/addInvenduVignette/${idc}/${vente}`;
                xhttp.open("GET",link,true);
                xhttp.send();
            }
            else

...
const   cmbEvents = document.getElementsByClassName('form-control EventId');
...
function checkforEmpty()
{
   if(index !=0)
   {
        for (let i = 0; i < index; i++) 
        {
            ...
            let idEvent = cmbEvents[i].options[cmbEvents[i].selectedIndex].id;
            if(Nbres[i].value =="" && idGuichet=='0' && idSection=='0' && idEvent=='0')
            {
...
function getJson3()
 {
    ...
    let idEvent = cmbEvents[selectedrow].options[cmbEvents[selectedrow].selectedIndex].id;
 ...
 function setEvent(array)
 {
     ...
     for(let i= 0 ; i<array.length ; i++)
     {
          ...
          // console.log(array[i].nom);
           ...
           //-${array[i].Arrivee}
           option.appendChild(t);
           cmbEvents[index].appendChild(option);
    }
